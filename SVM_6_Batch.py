...
def load_and_preprocess_images_labels(image_filenames, image_dir, label_dir, label_value=1, img_size=(img_size,img_size), augment_defects=False, id_to_images=None):
...
def generate_batches(file_list, batch_size):
    for i in range(0, len(file_list), batch_size):
        yield file_list[i:i + batch_size]


#### Load Data (validation and test for test) ####
# data_dir = r"C:\Users\marah\OneDrive\Documents\GitHub\datasets\DATASET"
...
batch_size = 720  # or any other size that fits your memory and processing needs
image_filenames_train = os.listdir(image_train_dir)
total_batches = len(image_filenames_train) // batch_size + (len(image_filenames_train) % batch_size > 0)

X_train, y_train = [], []
id_to_images_train = {}

# Add tqdm here for batch processing
for batch_filenames in tqdm(generate_batches(image_filenames_train, batch_size), total=total_batches, desc="Processing Training Batches"):
    X_batch, y_batch, defect_vials_batch = load_and_preprocess_images_labels(batch_filenames, image_train_dir, label_train_dir, label_value=1, augment_defects=True, id_to_images=id_to_images_train)
...
"""
batch_size = 90  # Set this based on your memory capacity
image_filenames_train = os.listdir(image_train_dir)
X_train, y_train = [], []
id_to_images_train = {}

for batch_filenames in tqdm(generate_batches(image_filenames_train, batch_size), desc="Processing Training Images"):
    X_batch, y_batch, defect_vials_batch = load_and_preprocess_images_labels(batch_filenames, image_train_dir, label_train_dir, label_value=1, augment_defects=True, id_to_images=id_to_images_train)
    X_train.extend(X_batch)
    y_train.extend(y_batch)
...
for img_dir, lbl_dir in tqdm(zip(image_valid_test_dirs, label_valid_test_dirs), total=len(image_valid_test_dirs), desc="Processing Test/Validation Data"):
    image_filenames_test = os.listdir(img_dir)
    for batch_filenames in generate_batches(image_filenames_test, batch_size):
        X_batch, y_batch, defect_vials_temp = load_and_preprocess_images_labels(batch_filenames, img_dir, lbl_dir, label_value=1, augment_defects=True, id_to_images=id_to_images_test)
...
actual_labels_by_vial_train = group_labels_by_vial(id_to_images_train, y_train)
